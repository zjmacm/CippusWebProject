...
$(document).ready(function(){
	...
	$('#user_info_normal').removeClass("change");
	$('.fun1').css(css2);
	$('#user_info_normal').css(css1);
	...
	$('#save').click(function(){
		...
		$.ajax({
			...
			type : "post",
			dataType : "json",
			data : {"user_id":$('#user_id').val(),"good_at":good_at,"prize":prize,"self_intro":self_intro,"tel":tel,"email":email},
			success : function(data) {
				if(data.result=="success")
	...
	$('#repassword_1').click(function(){
		...
		$.ajax({
			...
			type : "post",
			dataType : "json",
			data : {"user_id":$('#user_id').val(),"password":old_password},
			success : function(data) {
				if(data.result == "failed") {
	...
	$('#repassword_2').click(function(){
		...
		if(new_password1!=new_password2)
			...
			$.ajax({
				...
				type : "post",
				dataType : "json",
				data : {"user_id":$('#user_id').val(),"password":new_password1},
				success : function(data) {
					if(data.result == "failed") {
