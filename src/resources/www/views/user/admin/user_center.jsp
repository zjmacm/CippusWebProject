...
$(document).ready(function(){
	...
	$('#register_accept').removeClass("change");
	$('.fun1').css(css2);
	$('#register_accept').css(css1);
	...
	var grid=$("#register_list").flexigrid({  
        ...
        dataType: 'json',  
        colModel : [  
                {display:"学号", name:"number", width:100, sortable:false, align:'center'}, 
                {display:"姓名", name:"name", width:100, sortable:false, align:'center'}, 
                {display:"联系电话", name:"tel", width:100, sortable:false, align:'center'},  
                {display:"邮箱地址", name :"email", width:260, sortable:false, align:'center'},  
                {display:"申请时间", name :"register_time", width:100, sortable:false, align:'center'}
        ],  
        buttons : [  
	...
	function toolbar(com,grid){
		if(com == '查看详情') {
			if($('.trSelected',grid).length == 0)
                ...
                jcl.go('/CippusWebProject/getRegisterInfo?user_id='+id);
			}
		}
		else if(com == '删除申请') {
			if($('.trSelected',grid).length == 0)
                    ...
                    $.ajax({
            			...
            			success : function(data){
            			    if(data.result=='success'){
            			    	alert('已成功删除该申请！');
            			    	jcl.go('/CippusWebProject/admin/member_info');
            			    }
            			    else {
            			    	alert('操作失败,请稍后再试！');
            			    }
